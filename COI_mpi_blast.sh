...
# ── Collect input files ───────────────────────────────────────────────────────
ALL_FILES=($(ls "$WORKDIR"/$PATTERN 2>/dev/null))
FILES=()
EMPTY_FILES=()
...
if [ ${#EMPTY_FILES[@]} -gt 0 ]; then
    echo "WARNING: Skipping ${#EMPTY_FILES[@]} empty files:"
    for ef in "${EMPTY_FILES[@]}"; do echo "  - $ef"; done
fi

echo "Found $TOTAL_FILES non-empty files to process"
