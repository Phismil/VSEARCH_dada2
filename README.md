...
RC_FWD="GGRGGRTAWACWGTTCAWCCWGTWCC"
RC_REV="TGRTTYTTYGGNCAYCCNGARGTNTA"
...
mkdir -p merged unmergedf unmergedr
    ...
    vsearch --fastq_filter "${s}.merged.fastq" --fastq_maxee "$MAX_EE" --fastq_minlen "$MIN_LENGTH_MERGED" --fastq_maxlen "$PRIMER_LENGTH" --fastq_maxns 0 --fastaout "${s}.filtered.fasta" --fasta_width 0
...
cat all.tab_otus.merged.txt > merged.tab.txt  && tail -n +2 all.tab_otus.unmergedf.txt >> merged.tab.txt
